...
def save_plot(fig, filename, parent_directory=None, format=None, width=None, height=None, scale=2):
    """
    ...
    os.makedirs(os.path.join(parent_directory, "plots"), exist_ok=True)
    
    # Determine format from filename if not specified
    if format is None:
        format = filename.split('.')[-1].lower()
    
    full_path = os.path.join(parent_directory, "plots", filename)
...
def load_data(parent_directory):
    ...
    for file in data_files:
        try:
            ...
            if not match:
                raise ValueError(f"could not parse lattice size from filename '{filename}'")
            L = int(match.group(1))
...
def plot_raw_data(data, Tc_estimate):
    ...
    # Save the plot
    save_plot(fig, "ising_raw_data.png", width=1000, height=800, scale=2)
    save_plot(fig, "ising_raw_data.html")
...
def plot_scaling_collapse(scaling_data):
    ...
    # Save the plot
    save_plot(fig, "scaling_collapse.png", width=1000, height=500, scale=2)
    save_plot(fig, "scaling_collapse.html")
...
def export_data_for_external_analysis(data):
    ...
    try:
        ...
        out_path = os.path.join(out_dir, "ising_combined_data.csv")
        combined_df.to_csv(out_path, index=False)
        print(f"Exported combined data to '{out_path}'")
...
def main():
    ...
    fig_raw = plot_raw_data(data, Tc_estimate)
    ...
    fig_scaling = plot_scaling_collapse(scaling_data)
